...
select left('HelloWorld', 5) as `왼쪽 5개`;
select right ('HelloWorld', 5) as `오른쪽 5개`;
select substring('HelloWorld', 6, 5) as `W 포함 5개`;
...
select curtime();
select now();
insert into `member` values ('all2', '유관순', '010-1234-1012', '대리', 107, now());
...
#실습 4-16
select sum(`sale`) from `sales` as a join `member` as b on a.`uid` = b.`uid` where `name` = '김유신' and `year` = 2019;
...
select b.`name` as `직원이름`,
       ...
       b.`pos` as `직급`,
       a.`year` as `년도`,
       sum(`sale`) > 100000 as `매출 합`
from `sales` as a
join `member` as b on a.`uid` = b.`uid`
join `department` as c on b.`dep` = c.`depno`
group by `year` = 2019 and `sale` >= 50000 
order by `매출 합`
;